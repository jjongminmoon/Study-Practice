...
export default function AdjacentPostCard({ post, type }: Props) {
  ...
  return (
    <Link className="relative w-full bg-black max-h-56" href={`/posts/${post.path}`}>
      <Image
        ...
        height={100}
      />
      <div className=" group absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex justify-around items-center w-full text-white px-8">
        {type === "prev" && <FaArrowLeft className={ICON_CLASS} />}
        <div className="w-full text-center">
          <h3 className="text-3xl font-bold text-center">{post.title}</h3>
